...
import { Task, Folder, SuperFolder } from "./modules/factories.js";
...
taskEditConfirmBtn.addEventListener("click", (e) => {
  e.preventDefault();
...
function loadPresetFolders() {
  if (localStorage.getItem("folders")) {
    ...
    const task1 = Task(
      ...
      format(new Date(2024, 2, 19), "MMM do, yyyy"),
      "High",
    );
    const task2 = Task(
      "Vaccuum",
      "Clean 2nd floor",
      format(new Date(2024, 3, 20), "MMM do, yyyy"),
      "Medium",
    );
    const task3 = Task(
      "Finish project",
      "Iron out bugs from THAT project",
      format(new Date(2024, 1, 30), "MMM do, yyyy"),
      "Low",
    );

    const inboxFolder = Folder("Inbox", "inboxFolder");
    inboxFolder.addTask(task3);
    superFolder.addFolder(inboxFolder);
    appendFolder(inboxFolder);

    const chores = Folder("House Chores");
...
function recreateTaskObj(targetObj) {
  const { title, description, dueDate, priority, myTaskUuid, completeState } =
    targetObj;
...
function recreateFolderFromObject(genericObj) {
  ...
  const folderDupe = Folder(title, myFolderUuid);
